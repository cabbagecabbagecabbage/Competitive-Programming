...
/*
FINE I'LL STOP THROWING SEGTREES SMH MY HEAD
*/


const int nax = (1 << 23) + 5, mod = 1e9 + 7; const double eps = 1e-9;
int n,k,pre[nax];
...
int32_t main() {
	...
	f2(1,n+2){
		...
		pre[i] = d.front().second;
		int val = d.front().first + a;
		while (d.back().first >= val) d.pop_back();
		d.push_back({val,i});
	}
	out(d.back().first);
	vector<int> ans;
	for (int i = n+1; i != pre[i]; i = pre[i]) ans.eb(i);
	for (int i = ans.size() - 1; i > 1; --i) cout << (ans[i]) << " "; out(ans[1]);
